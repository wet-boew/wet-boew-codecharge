...
//Initialize Common Variables @0-FE5CCB1A
...
CCConvertDataArrays();
$CCProjectStyle = "";
$CCProjectDesign = "theme-gc-intranet";
...
$CCSDesign = "";
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", '2RTT2ylqA9ja40Rf');
...
//myproject Connection Class @-73093994
class clsDBmyproject extends DB_Adapter
{
    function clsDBmyproject()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["myproject"]);
        parent::Initialize();
...
//End myproject Connection Class
...
//CCSelectProjectDesign @0-7E5ECAFE
function CCSelectProjectDesign() {
    ...
    if ($QueryDesign) {
        ...
        CCSetCookie("myproject_design", $CCProjectDesign, time() + 31536000);
        return;
    }
    if (CCSetProjectDesign(CCGetSession("design")))
        return;
    CCSetProjectDesign(CCGetCookie("myproject_design"));
}
...
//CCLoginUser @0-953E1C64
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBmyproject();
    $SQL = "SELECT user_id, group_id, user_password FROM users WHERE user_login=" . $db->ToSQL($login, ccsText) . " AND user_password=MD5(CONCAT('2RTT2ylqA9ja40Rf'," . $db->ToSQL($password, ccsText) . "))";
    $db->query($SQL);
